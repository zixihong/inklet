...
async function main() {
  ...
  if (values.width) options.width = parseInt(values.width, 10)
  if (values.chars) options.characterSet = values.chars
  if (values.aspect) options.charAspectRatio = parseFloat(values.aspect)
  if (values.invert) options.invert = true
  if (values["remove-bg"]) options.removeBackground = true
  ...
  if (values.print) {
    ...
    if (values.output) {
      ...
      console.log(
        `Written ${data.cols}x${data.rows} ASCII (${data.chars.length} chars, ${data.palette.length} colors) to ${outPath} (${sizeMB} MB)`
      )
    }
